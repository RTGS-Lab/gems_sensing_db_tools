...
def parse_sensor_messages(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in df.iterrows():
        try:
            ...
            logger.warning(f"Failed to parse message for row {row.get('id', 'unknown')}: {e}")
            continue
...
def get_available_parameters(df: pd.DataFrame, max_depth: int = 5) -> Dict[str, Set[str]]:
...
def _extract_all_parameters(data: Union[Dict, List, Any], prefix: str = "", max_depth: int = 5) -> Set[str]:
...
def extract_time_series_data(
    ...
    if 'parsed_message' not in df.columns:
        raise ValidationError("DataFrame must have 'parsed_message' column. Run parse_sensor_messages() first.")
    ...
    for node_id in df['node_id'].unique():
        ...
        for _, row in node_data.iterrows():
            if 'parsed_message' in row and row['parsed_message']:
                value = extract_parameter_from_json(row['parsed_message'], parameter_path)
