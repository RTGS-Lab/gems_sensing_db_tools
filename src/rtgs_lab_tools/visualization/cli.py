...
def create(ctx, file, parameter, node_id, multi_param, output_dir, output_file, 
          output_format, list_params, title, no_markers, verbose, log_file, 
          no_git_log, note):
    ...
    try:
        ...
        if multi_param:
            ...
            operation = f"Create time series plot for {parameter} from {Path(file).name}"
...
def list_parameters(ctx, file, verbose, log_file, no_git_log, note):
    ...
    try:
        ...
        cli_ctx.log_error("Parameter listing error", e, {'file': file, 'note': note}, __file__)
        raise
