...
try:
    ...
    logger.warning("Gridded data processing requires xarray. Install with: pip install rtgs-lab-tools[climate]")
...
def process_era5_data(
    ...
    try:
        ...
        if spatial_subset:
            lat_slice = slice(spatial_subset.get('lat_max'), spatial_subset.get('lat_min'))
            lon_slice = slice(spatial_subset.get('lon_min'), spatial_subset.get('lon_max'))
        ...
        if temporal_aggregation:
            if temporal_aggregation == 'daily':
                ...
                raise ValidationError(f"Unknown temporal aggregation: {temporal_aggregation}")
...
def extract_time_series(
    ...
    try:
        ...
        df = df[['time', variable]].rename(columns={'time': 'timestamp', variable: 'value'})
