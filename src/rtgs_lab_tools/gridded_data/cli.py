...
from ..core.cli_utils import (
    ...
    parse_area_bounds, parse_comma_separated_list
)
...
def era5(ctx, variables, start_date, end_date, area, output_file, pressure_levels, 
         time_hours, list_variables, process, verbose, log_file, no_git_log, note):
    ...
    try:
        ...
        pressure_list = None
        if pressure_levels:
            pressure_list = parse_comma_separated_list(pressure_levels, int, "pressure levels")
...
@click.option('--temporal-aggregation', type=click.Choice(['daily', 'monthly']), help='Temporal aggregation')
@click.option('--spatial-subset', help='Spatial subset as "lat_min,lat_max,lon_min,lon_max"')
@add_common_options
@click.pass_context
@handle_common_errors("era5-processing")
def process_era5(ctx, file_path, variables, temporal_aggregation, spatial_subset, 
    ...
    try:
        ...
        if spatial_subset:
            try:
                lat_min, lat_max, lon_min, lon_max = [float(x.strip()) for x in spatial_subset.split(',')]
                spatial_dict = {
                    'lat_min': lat_min,
                    'lat_max': lat_max,
                    'lon_min': lon_min,
                ...
                click.echo("Error: Spatial subset must be 'lat_min,lat_max,lon_min,lon_max'")
                sys.exit(1)
