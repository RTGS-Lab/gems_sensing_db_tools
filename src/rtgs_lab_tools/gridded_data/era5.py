...
try:
    ...
    CDS_AVAILABLE = False
    logger.warning("ERA5 functionality requires cdsapi and xarray. Install with: pip install rtgs-lab-tools[climate]")
...
class ERA5Client:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        if not self.api_key:
            raise APIError("CDS API key not found. Set CDS_API_KEY environment variable or configure ~/.cdsapirc")
    ...
    def download_era5_reanalysis(
        ...
        try:
            ...
            if end_dt > datetime.now() - timedelta(days=5):
                raise ValidationError("ERA5 data has ~5 day delay. Use more recent data.")
            ...
            logger.info(f"Downloading ERA5 data: {variables} from {start_date} to {end_date}")
