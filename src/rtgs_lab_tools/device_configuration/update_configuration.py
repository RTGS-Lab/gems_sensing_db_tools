...
class ParticleConfigUpdater:
    ...
    def __init__(self, enable_git_logging: bool = True, repo_path: str = None, config: Optional[Config] = None):
        if config is None:
            config = Config()
        self.config = config
    ...
    def get_configuration_uids(self, device_id: str, session: Optional[requests.Session] = None) -> Tuple[Optional[int], Optional[int], bool]:
        ...
        for attempt in range(self.uid_check_retries):
            ...
            success, system_uid, timeout = self.client.call_function(device_id, "getSystemConfig", session=session)
            if not success:
            ...
            success, sensor_uid, timeout = self.client.call_function(device_id, "getSensorConfig", session=session)
            if not success:
    ...
    def verify_configuration_uid(self, device_id: str, expected_system_uid: int, expected_sensor_uid: int, 
                                session: Optional[requests.Session] = None) -> Tuple[bool, Optional[int], Optional[int]]:
    ...
    def update_device_config(self, device_id: str, config: Dict[str, Any], expected_system_uid: int, 
                           expected_sensor_uid: int, config_json_str: str) -> Dict[str, Any]:
        ...
        for attempt in range(self.max_retries):
            ...
            success, response, timeout = self.client.call_function(device_id, "updateConfig", config_json_str, session)
            ...
            if timeout:
                ...
                if not self.client.wait_for_device_online(device_id, self.online_check_timeout, session):
