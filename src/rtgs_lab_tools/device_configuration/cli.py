...
def update_config(ctx, config, devices, output, max_retries, restart_wait, online_timeout,
                 max_concurrent, dry_run, no_particle_git_log, verbose, log_file, no_git_log, note):
    ...
    try:
        ...
        if dry_run:
            ...
            cli_ctx.logger.info(f"Would update {len(device_ids)} devices with configuration:")
        ...
        updater = ParticleConfigUpdater(
            enable_git_logging=enable_particle_git_log,
            config=app_config
        )
